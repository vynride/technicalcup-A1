...
const Box = ({ children, className = "" }) => (
  <div
    className={`p-6 rounded-[1.5rem] border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all duration-200 ${className}`}
  >
    {children}
  </div>
);
...
export default function Dashboard() {
  ...
  const selectedDay = selectedDate.toLocaleDateString("default", {
    weekday: "long",
    month: "short",
    day: "numeric",
  });
      ...
      <main className="mx-auto space-y-6 px-2 md:px-4">
        <div className="flex flex-col lg:flex-row gap-6 items-end justify-between">
          <div className="w-full overflow-hidden">
            <h2 className="text-6xl md:text-7xl font-extrabold tracking-tight mb-2">
              Weekly Schedule
            </h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-9 gap-6">
                ...
                <div className="bg-[#1A1A1A] text-white p-5 rounded-[1.5rem] border-2 border-black flex-1">
                  ...
                  <div className="grid grid-cols-7 gap-2 text-center text-sm text-gray-500 mb-2">
                    {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
                      <div
                        key={i}
                        className="py-2 font-semibold text-gray-400 text-base"
                      >
                        {d}
                      </div>
                    ))}
                ...
                <div className="bg-[#1A1A1A] text-white p-6 rounded-[1.5rem] border-2 border-black flex-1 flex flex-col gap-4">
                  <div>
                    ...
                    <div className="flex gap-3">
                      <button className="px-5 py-2 text-white rounded-full text-sm">
                        Lecture
                      </button>
                    </div>
                  </div>
                  <div className="flex flex-col gap-3">
                    {planItems.map((item) => (
                      <div
                        key={item.id}
                        className={`${item.accent} text-black p-5 rounded-2xl border border-black/20`}
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-xs font-bold mt-1 uppercase text-gray-700">
                              Deadline
                            </p>
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            <Box className="bg-[#D0BCFF] h-fit flex flex-col gap-4">
              ...
              <div className="space-y-3">
                {tasks.map((task) => (
                  <div
                    key={task.id}
                    className="bg-white border-2 border-black rounded-xl p-4 flex flex-col shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                  >
              ...
              <div className="space-y-3">
                {resources.map((resource) => (
                  ...
                  >
                    ...
                    <Download
                      size={20}
                      className="text-black flex-shrink-0 ml-3 group-hover:scale-110 transition-transform"
                    />
                  </button>
                ))}
              </div>
